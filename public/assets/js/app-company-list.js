...
$(function () {
  ...
  var selectdtypees = $('.select2tipoes');

  if (selectdtypees.length) {
    var $this = selectdtypees;
    $this.wrap('<div class="position-relative"></div>').select2({
      placeholder: 'Seleccionar Tipo Establecimiento',
      dropdownParent: $this.parent()
    });
  }
  ...
  var select2tipoesedit = $('.select2tipoesedit');

  if (select2tipoesedit.length) {
    var $this = select2tipoesedit;
    $this.wrap('<div class="position-relative"></div>').select2({
      placeholder: 'Seleccionar Tipo Establecimiento',
      dropdownParent: $this.parent()
    });
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Detalles empresa ' + data[2];
            }
          }),
          type: 'column',
